...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, level, camera, mixer, scale: int = 100):
        ...
        self.ground_ticks = pygame.time.get_ticks()
    ...
    def paused(self, value: bool):
        if not value:
            self.yTicks += pygame.time.get_ticks() - self.pause_ticks
            self.ground_ticks += pygame.time.get_ticks() - self.pause_ticks
        else:
            # Set pause timer.
    ...
    @staticmethod
    def get_velocity(acceleration: float, ticks: int, velocity: float):
        """
        Return the velocity given the acceleration, initial velocity and time.
        Uses the equation - v = a * dt + u 
        """
        return acceleration * ((pygame.time.get_ticks() - ticks) / 1000) + velocity
    ...
    def update_particles(self, velx, vely):
        ...
        for i, particle in enumerate(self.particles):
            if particle.rect.width != 0:
                ...
                self.particles[i].image = pygame.surface.Surface(
                    (self.particles[i].rect.width,
    ...
    def update(self):
        ...
        if not self.paused:
            ...
            if self.death_ticks > 0:
                ...
                if (pygame.time.get_ticks() - self.death_ticks) > self.death_duration:
                    self.yTicks = pygame.time.get_ticks()
                    self.ground_ticks = pygame.time.get_ticks()
            ...
            if self.death_ticks == 0:
                if self.level.failed(self):
                    self.deaths += 1
                    self.death_ticks = pygame.time.get_ticks()
                    self.mixer["destroy"].play()
                    self.image = self.images["exploded"].convert_alpha()
                else:
                    self.ui.scroll(self)
            else:
                if 0 < (pygame.time.get_ticks() - self.death_ticks) < self.death_duration:
