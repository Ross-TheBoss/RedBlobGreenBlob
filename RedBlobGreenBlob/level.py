...
{},
{},
{"type":TOPTILE,"flags":TILECOLLIDE,"files":["grass.png","grassc.png","grassp.png"]},
...
{"type":ANITILE,"flags":TILEDEADLY | TILEALPHA,"files":["lava.png","lava1.png","lava2.png","lava3.png","lava4.png"]},
{"type":VARTILE,"flags":TILECOLLIDE,"files":["stone.png","stone1.png","stone2.png"]}
]

for tile in TILES:
    if "files" in tile:
        tile.update({"files":list(map(lambda file: os.path.join(IMGDIR,file),tile["files"]))})
...
class Level(pygame.sprite.AbstractGroup):
    def __init__(self,file,size):
        ...
        dirtiles = list(map(lambda x: self.parse_tile(content,chr(x[0] + 40)) if x[1].get("type",-1) == TOPTILE else None,
                            enumerate(TILES)))

        # Tiles that won't be collided with. (air, background decor, etc. )
        bgtiles = list(filter(None,map(lambda x: False if x[1].get("flags",0) & TILECOLLIDE else chr(x[0] + 40),
                                       enumerate(TILES))))
        ...
        for i,block in enumerate(content):
            ...
            y = i // self.width
            if not block in [" ","\n"]:
                if ord(block) - 40 >= 0 and self.tiles[ord(block) - 40].get("type",-1) == TOPTILE:
                ...
                if tile["flags"] & TILECOLLIDE:
                    tile["flags"] = (tile["flags"] - TILECOLLIDE) | \
                                    (TILECOLLIDE & int(self.has_collisions(bgtiles, content, i)))
                
                spriteTile = Tile(x*self.size,y*self.size,tile,self.size,direction)
    ...
    def parse_tile(self,content,tile):
        ...
        for i,block in enumerate(leveltiles):
            if block != " ":
                ...
                below = leveltiles[i + self.width] if i + self.width < len(leveltiles) else "\x00"
